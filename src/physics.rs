...
use crate::components::{Collider, CollisionPair, FoliageWind, Position, RigidBody};
...
pub fn physics_system(
    ...
    time_s: f32,
    jobs: &JobSystem,
    foliage_wind_enabled: bool,
    foliage_wind_this_frame: bool,
) -> Vec<CollisionPair> {
    ...
    // Wind sway for foliage/tree entities.
    if foliage_wind_enabled && foliage_wind_this_frame {
    ...
    let collidables: Vec<(Entity, f32, f32, f32, f32)> = world
        .query::<(Entity, &Position, &Collider)>()
        .iter()
        .map(|(e, pos, col)| (e, pos.x, pos.y, col.half_w, col.half_h))
        .collect();

    // Parallel broad-phase pair test (read-only), then sequential resolution.
    let colliding_pairs: Vec<(usize, usize)> = jobs.install(|| {
        (0..collidables.len())
            .into_par_iter()
            .map(|i| {
                let mut local_pairs = Vec::new();
                let (_, ax, ay, ahw, ahh) = collidables[i];
                for j in (i + 1)..collidables.len() {
                    let (_, bx, by, bhw, bhh) = collidables[j];
    ...
    let mut collisions: Vec<CollisionPair> = Vec::with_capacity(colliding_pairs.len());

    // Resolve collisions sequentially to keep ECS writes deterministic and safe.
